...
| Build (includes typecheck) | `bun run build` |
| Lint | `bun run lint` |
...
Verification order: `bun run lint && bun run typecheck` before committing. `bun run build` will fail if typecheck fails (runs `tsc -b && vite build`).
...
src/
  features/         # Feature modules (blog, github, home, hw-list, math-games)
    blog/
      ...
      lib/          # Business logic (blog loading, TOC generation)
...
- Dev frontend proxies `/api` to `https://wwchun.top` (see `vite.config.ts`) — API features need the remote server or local server running

## Deployment

- **Frontend CI** (`.github/workflows/deploy.yml`): on push to `main`, runs `bun run build`, SCPs `dist/` to VPS at `/var/www/wwchun.top`, reloads nginx
